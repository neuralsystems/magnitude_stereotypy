...
switch p.Results.file_type
	case 'pdf'
		export_fig(h_fig, file_path, '-pdf', '-painters', '-nocrop');
	
	case 'eps'
		print(h_fig, file_path, '-depsc', '-painters', '-r300', '-noui');

	case 'svg'
		print(h_fig, file_path, '-dsvg', '-painters');
		
	case 'png'
		export_fig(h_fig, file_path, '-png', '-opengl', '-r300', '-nocrop', '-transparent');
