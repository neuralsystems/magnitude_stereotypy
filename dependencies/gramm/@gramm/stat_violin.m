...
my_addParameter(p,'outline',false)
my_addParameter(p,'mean',true)
my_addParameter(p,'points',true)
parse(p,varargin{:});
...
for ind_x=1:length(uni_x)
    ...
    if ~isempty(ysel)
        ...
        %Draw points
        if params.points
            xsel = zeros(size(ysel));
            for iYsel = 1:length(ysel)
                dens_ysel = dens{ind_x}(finddenspos(ysel(iYsel), dens_pos{ind_x}));
                xsel(iYsel) = boxmid(ind_x) + rand(1) * dens_ysel * 2 - dens_ysel;
            end
            points=line(xsel,ysel,'LineStyle','none','Marker','o','MarkerEdgeColor','none','markerSize',5,'MarkerFaceColor',face_color);
            set_alpha(points,1,face_alpha);
        end
        ...
        %Draw errorbar
        ci=nanstd(ysel)./sqrt(sum(~isnan(ysel)));
        yci=bsxfun(@plus,mean_ysel,[-ci;ci]);
        mean_hndl=plot([boxmid(ind_x) boxmid(ind_x)],[yci(1) yci(2)],'k-','LineWidth',2);
        
        %Draw patch
        patches(ind_x)=patch(xpatch,ypatch,[1 1 1],'EdgeAlpha',0,'FaceColor',face_color,'EdgeColor','none','FaceAlpha',face_alpha);
...
obj.results.stat_violin{obj.result_ind,1}.point_handle=points;
obj.results.stat_violin{obj.result_ind,1}.mean_handle=mean_hndl;

end
function pos = finddenspos(y, dens_pos)
[~, pos_sort] = sort(abs(dens_pos-y));
pos = pos_sort(1);
end